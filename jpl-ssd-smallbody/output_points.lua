...
local julianDay = Julian.fromCalendar(require 'ext.timer'.gmtime())
print('Evaluating for Julian day', julianDay)
...
OutputToPoints.julianDay = julianDay
...
function OutputToPoints:processBody(body)
	...
	elseif orbitType == 'elliptic' then
		...
		if isComet then
			...
			local timeSinceLastPeriapsisCrossing = julianDay - timeOfPeriapsisCrossing
			meanAnomaly = timeSinceLastPeriapsisCrossing * 2 * math.pi / orbitalPeriod
			
			body.meanAnomalyAtEpoch = meanAnomaly - 2 * math.pi / orbitalPeriod * (julianDay - body.epoch) 
		elseif isAsteroid then
			...
			meanAnomaly = body.meanAnomalyAtEpoch + 2 * math.pi / orbitalPeriod * (julianDay - body.epoch)
		else
		--	error 'here'
		end
	else
