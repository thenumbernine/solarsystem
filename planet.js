var Planet = makeClass({
	...
	calcKeplerianOrbitalElements : function(useVectorState) {
			...
			*/
			...
			if (orbitType === 'parabolic') {
				...
				if (this.isComet) {
					...
					meanAnomaly = this.sourceData.meanAnomalyAtEpoch + 2 * Math.PI / orbitalPeriod * (initJulianDate - this.sourceData.epoch);
				} else if (this.isExoplanet) {
...
*/
			...
			vec3.copy(solarSystem.initPlanets[planetIndex].pos, this.pos);
			vec3.copy(solarSystem.initPlanets[planetIndex].vel, this.vel);
...
//override for Earth
if (this.orbitalPeriod !== undefined) orbitalPeriod = this.orbitalPeriod;
		...
		integration can be simulated along Keplerian orbits using the A and B vectors ...
			...
			this position and velocity can then be computed using A and B's evaluation and derivatives
