		...
		xhr.onload = function(e) {
			...
			for (var j = 0; j < floatBuffer.length; ++j) {
				...
				if (j%3 == 2) {
					...
					if (thiz.closestDistInM === undefined) {
						thiz.closestDistInM = len;
					} else {
						thiz.closestDistInM = Math.min(len, thiz.closestDistInM); 
					}
				}
			...
			thiz.buffer = new glutil.ArrayBuffer({data : floatBuffer});
