...
class NewtonApproximateMetric extends Metric {
	...
	*/
	calcGravity(accel, pos) {
		for (let planetIndex = 0; planetIndex < cfg.orbitStarSystem.planets.length; ++planetIndex) {
			...
			const planet = cfg.orbitStarSystem.planets[planetIndex];
			if (planet.isBarycenter) continue;
			if (planet.mass === undefined) continue;

			const x = pos[0] - planet.pos[0];
			const y = pos[1] - planet.pos[1];
			const z = pos[2] - planet.pos[2];
			const r = Math.sqrt(x*x + y*y + z*z);
			const r2 = r * r;
			const accelMagn = gravitationalConstant * planet.mass / r2;
	...
	*/
	calcTidal(accel, pos, srcPlanet) {
		const nx = pos[0] - srcPlanet.pos[0];
		const ny = pos[1] - srcPlanet.pos[1];
		const nz = pos[2] - srcPlanet.pos[2];
	...
	*/
	calcGravity(accel, pos) {
		for (let planetIndex = 0; planetIndex < cfg.orbitStarSystem.planets.length; ++planetIndex) {
			...
			const r = Math.sqrt(r2);
			const r3 = r * r2;
			const R = 2. * planet.mass * kilogramsPerMeter;
...
class KerrMetric extends Metric {
	calcGravity(accel, pos) {
		for (let planetIndex = 0; planetIndex < cfg.orbitStarSystem.planets.length; ++planetIndex) {
			...
			const cosTheta = r2D / r;

			const M = planet.mass * gravitationalConstant / metersPerUnits.ls / metersPerUnits.ls;	//mass, in m

			const rotationPeriod = planet.rotationPeriod !== undefined ? planet.rotationPeriod : 0;
			const angularVelocity = 2. * Math.PI / ( rotationPeriod * (60. * 60. * 24.)) / metersPerUnits.ls;	// angular velocity, in 1/m
			const inertia = 2./5. * M * planet.radius * planet.radius;					// moment of inertia about a sphere, in m^3
			const angularMomentum = inertia * angularVelocity;						// angular momentum in m^2
			const a = M == 0 ? 0 : angularMomentum / M;											// m
...

{
	...
	cfg.calcMetricForce = function(x, planet) {
		...
		case 'Tangent Gravitational':
		case 'Tangent Total':
			const dot = accel[0] * norm[0] + accel[1] * norm[1] + accel[2] * norm[2];
			const dx = accel[0] - norm[0] * dot;
			const dy = accel[1] - norm[1] * dot;
			const dz = accel[2] - norm[2] * dot;
