...
local static = json.decode(path'static-vars.json':read():match(staticIntro..'(.*)'))

local dynamicIntro = 'horizonsDynamicData = '
local dynamic = json.decode(path'dynamic-vars.json':read():match(dynamicIntro..'(.*)'))
...
path'static-vars.json':write(staticIntro..json.encode(static, {indent=true})..';')
path'dynamic-vars.json':write(dynamicIntro..json.encode(dynamic, {indent=true})..';')
